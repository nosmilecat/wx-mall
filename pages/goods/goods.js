...
Page({
    ...
    getGoodsInfo: function() {
        ...
        util.request(api.GoodsDetail, { id: that.data.id }).then(function(res) {
            if (res.code === 200) {
                that.setData({
                    goods: res.result.info,
                    gallery: res.result.gallery,
                    attribute: res.result.attribute,
                    issueList: res.result.issue,
                    comment: res.result.comment,
                    brand: res.result.brand,
                    specificationList: res.result.specificationList,
                    productList: res.result.productList,
                    userHasCollect: res.result.userHasCollect
                });
                //设置默认值
                that.setDefSpecInfo(that.data.specificationList);
                if (res.result.userHasCollect == 1) {
                    that.setData({
                ...
                WxParse.wxParse('goodsDetail', 'html', res.result.info.goodsDesc, that);
    ...
    getCheckedProductItem: function(key) {
        return this.data.productList.filter(function(v) {
            console.log(v.goodsSpecificationIds, key, v.goodsSpecificationIds.indexOf(key))
    ...
    onLoad: function(options) {
        ...
        util.request(api.CartGoodsCount).then(function(res) {
            if (res.code === 200) {
                that.setData({
                    cartGoodsCount: res.result.cartTotal.goodsCount
                });
            ...
            util.request(api.CartAdd, { goodsId: this.data.goods.id, number: this.data.number, productId: checkedProduct[0].id }, 'POST', 'application/json')
                .then(function(res) {
                    ...
                    if (_res.code === 200) {
                        ...
                        that.setData({
                            openAttr: !that.data.openAttr,
                            cartGoodsCount: _res.result.cartTotal.goodsCount
                        });
                        if (that.data.userHasCollect == 1) {
                            that.setData({
                                'collectBackImage': that.data.hasCollectImage
